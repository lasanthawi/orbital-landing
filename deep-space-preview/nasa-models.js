...
function hideProcedural(scene){
  scene.children.forEach(obj=>{
    ...
    if(obj?.geometry?.type==='TubeGeometry') obj.visible=false;
  });
}

function fitModel(root,targetSize){
  const box=new THREE.Box3().setFromObject(root),size=new THREE.Vector3();box.getSize(size);
  root.scale.multiplyScalar(targetSize/(Math.max(size.x,size.y,size.z)||1));
  const box2=new THREE.Box3().setFromObject(root),center=new THREE.Vector3();box2.getCenter(center);root.position.sub(center);
  root.traverse(o=>{if(o.isMesh){o.castShadow=true;o.receiveShadow=true;if(o.material){o.material.envMapIntensity=.9;o.material.needsUpdate=true;}}});
  return root;
}
...
function init(scene){
  initialized=true;hideProcedural(scene);
