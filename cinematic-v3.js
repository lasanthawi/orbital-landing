...
// Each encounter is an establishing shot followed by a restrained orbital arc.
// Adjacent points remain on the same side of each subject, avoiding chords through
// the centre and preserving a readable full silhouette throughout the move.
const shots=[
...
function sample(p){
  ...
  return {
    pos:V(...a.pos).lerp(V(...b.pos),q),
    look:V(...a.look).lerp(V(...b.look),q),
    roll:THREE.MathUtils.lerp(a.roll,b.roll,q)
  };
}
...
function enforceEnvelope(pos){
  for(const s of subjects){
    ...
    if(d<s.r){
      if(d<.001) off.set(1,.15,.3);
      pos.copy(s.c).add(off.normalize().multiplyScalar(s.r));
    }
  }
  return pos;
}
...
THREE.WebGLRenderer.prototype.render=function(scene,camera){
  ...
  widePos.lerp(s.pos,posA*.30);
  wideLook.lerp(s.look,lookA*.26);
  enforceEnvelope(widePos);
